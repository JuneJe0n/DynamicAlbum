...
#############################################################
def to_uint8_hwc_frames(frames):
    if torch.is_tensor(frames):
        arr = frames.detach().cpu().float().numpy()
    else:
        arr = np.asarray(frames, dtype=np.float32) if not isinstance(frames, np.ndarray) else frames

    # 5D (B,F,H,W,C) → (F,H,W,C)
    ...
    if arr.ndim == 4:
        if arr.shape[-1] in (1, 3, 4):
            frames_iter = [arr[i] for i in range(arr.shape[0])]  # (F,H,W,C)
        elif arr.shape[1] in (1, 3, 4) and arr.shape[-1] not in (1,3,4):
            arr = np.transpose(arr, (0, 2, 3, 1))               # (F,C,H,W) -> (F,H,W,C)
            frames_iter = [arr[i] for i in range(arr.shape[0])]
        else:
            frames_iter = [arr]
    elif arr.ndim == 3:
        frames_iter = [arr]
    else:
        frames_iter = list(frames) if isinstance(frames, (list, tuple)) else [arr]

    out = []
    for fr in frames_iter:
        if torch.is_tensor(fr):
            fr = fr.detach().cpu().float().numpy()
        fr = np.asarray(fr)
        ...
        if fr.dtype != np.uint8:
            maxv = float(fr.max()) if fr.size else 1.0
            if maxv <= 1.0:
...
def eval(pipeline, vae_processor, validation_data, out_file, index, forward_t=25, preview=True):
    ...
    ############ (H,W,3) uint8 RGB 프레임으로 표준화 #################

    frames_rgb = to_uint8_hwc_frames(video_frames)
    ...
    if preview:
        fps = validation_data.get('fps', 8)
        frames_uint8 = to_uint8_hwc_frames(video_frames)
        imageio.mimwrite(out_file, frames_uint8, duration=int(1000/fps), loop=0)
        imageio.mimwrite(out_file.replace('gif', '.mp4'), frames_uint8, fps=fps)
    ...
    frames_rgba = [np.dstack([fr, alpha]) for fr in frames_rgb]  
    alpha_dir = os.path.splitext(out_file)[0] + "_alpha_frames"
    os.makedirs(alpha_dir, exist_ok=True)
    for i, fr in enumerate(frames_rgba):
        Image.fromarray(fr, mode="RGBA").save(os.path.join(alpha_dir, f"{i:04d}.png"))
    print(f"[alpha] saved RGBA PNG sequence to: {alpha_dir}")

    # ffmpeg가 있으면 알파 포함 동영상도 생성 가능:
    # WebM(alpha): ffmpeg -framerate {fps} -i {alpha_dir}/%04d.png -c:v libvpx-vp9 -pix_fmt yuva420p {out_file.replace('.gif','_alpha.webm')}
    # APNG:        ffmpeg -framerate {fps} -i {alpha_dir}/%04d.png -plays 0 {out_file.replace('.gif','_alpha.apng')}

    frames_bgr = [cv2.cvtColor(fr, cv2.COLOR_RGB2BGR) for fr in frames_rgb]
...
def main_eval(
    ...
    **kwargs
    ...
    noise_scheduler, tokenizer, text_encoder, vae, unet = load_primary_models(
        pretrained_model_path, motion_strength=motion_strength
    )
    vae_processor = VaeImageProcessor()
    ...
    batch_eval(
        unet, text_encoder, vae, vae_processor, pretrained_model_path,
        validation_data, "output/demo", True
    )
    #############################
